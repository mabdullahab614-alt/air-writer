...
def process_frame(frame, color_choice, clear_canvas):
    ...
    if res.multi_hand_landmarks:
        ...
        if all_up:
            current_mode = "ERASE"
            gesture_text = "ERASING - All fingers up"
            prev_x, prev_y = 0, 0
        elif index_up and middle_up:
            current_mode = "MOVE"
            gesture_text = "MOVING - 2 fingers up"
            prev_x, prev_y = 0, 0
        elif index_up:
            current_mode = "DRAW"
            gesture_text = "DRAWING - Index finger up"

        if current_mode == "DRAW":
            if prev_x == 0 and prev_y == 0:
                prev_x, prev_y = ix, iy
            cv2.line(canvas, (prev_x, prev_y), (ix, iy), current_color, 8)
            prev_x, prev_y = ix, iy
    ...
    cv2.putText(merged, f"{gesture_text}  |  Mode: {current_mode}",
...
with gr.Blocks(title="Air Writer") as app:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            clear_btn = gr.Checkbox(label="Clear Canvas", value=False)
            gr.Markdown("""
---
### Gesture Guide
| Gesture | Action |
|---------|--------|
| 1 finger (index) | Draw |
| 2 fingers (index+middle) | Move (lift pen) |
...
**BS AI @ UMT Lahore**
    ...
    <div style="text-align:center; padding:16px; color:rgba(0,245,255,0.3);
        ...
        AIR WRITER - BUILT BY ABDULLAH JAVID - BS AI @ UMT LAHORE
    </div>
    """)
